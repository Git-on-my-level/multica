...
multica issue create --title "Promoted task" --client-key sha256:<64-lowercase-hex>
...
`--client-key` makes promotion retries authority-idempotent within the selected
workspace. An exact retry returns the existing issue; reusing the key with a
changed title, description, assignment, dates, labels, attachments, or other
create semantics returns `409 issue_client_key_conflict`. Generate the key from
a stable, non-secret promotion identity; Multica stores only its SHA-256 digest
and a separate digest of the normalized create request, never the raw brief.
The key remains consumed if the issue is later deleted, so a delayed retry
cannot silently recreate intentionally removed work.

Client-key creates that would immediately dispatch an agent or squad are
rejected with `422 issue_client_key_assigned_dispatch_unsupported`. Issue
creation and task enqueue are not one database transaction, so accepting that
combination would leave a commit-to-enqueue crash window. Create unassigned, or
create assigned in `backlog` and promote it separately.
...
| `event` | Replay or watch committed workspace events for automation |
...
For automation that reacts to workspace changes, replay the committed event stream and persist the returned `next_cursor` only after processing the page:

```bash
multica event list --cursor 0 --limit 100
multica event watch --cursor <last-processed-sequence>
multica --profile staging --workspace-id <workspace-uuid> event watch --cursor <cursor>
```

`event watch` prints one compact JSON object per line and stays quiet when nothing changed. The cursor tracks a workspace-wide sequence. With `--type` filters, the returned cursor also binds the normalized filter set and the server rejects reuse with different filters. A `cursor_expired` (`410 Gone`) response means retained history no longer reaches that position; restart from the returned `oldest_cursor` and reconcile current aggregate state before continuing. Event payloads contain bounded routing/state metadata; fetch the aggregate through its normal API when full content is needed.

Server operators can preview an explicit retention cutoff, then run bounded
prefix deletion. Nothing is deleted unless both a positive retention duration
and `--apply` are supplied:

```bash
cd server
go run ./cmd/prune_workspace_events --retention 720h
go run ./cmd/prune_workspace_events --retention 720h --batch-size 1000 --max-batches 100 --apply
```

Schedule the `--apply` form at the deployment layer if bounded retention is
desired. Re-run when the JSON summary reports `has_more: true`. The pruner
serializes with workspace appends and deletes only an age-eligible contiguous
sequence prefix, preserving cursor/high-water invariants.
...
| `event` | `list` | Read one bounded page from the durable workspace event stream | `--cursor`, `--limit`, `--type` (repeatable) |
| | `watch` | Poll the stream and print one compact JSON event per line | `--cursor`, `--limit`, `--type` (repeatable), `--interval` |
