...
| `multica issue url <id>` | Print the configured, workspace-qualified web URL for an issue |
| `multica issue inspect <id> --output json` | Read an ownership, run, PR-handoff, and safe-action snapshot |
| `multica issue route --title "..." [--repo <url>] [--project <id>] --output json` | Propose a project, duplicate, owner, and available agent without changing anything |
...
| `multica context --output json` | Show non-secret live workspace, project, repository, and routing-agent context |
