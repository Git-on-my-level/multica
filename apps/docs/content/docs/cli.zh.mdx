...
| `multica issue url <id>` | 输出基于当前配置、包含工作区的 issue 网页链接 |
| `multica issue inspect <id> --output json` | 查看负责人、运行、PR 交接与安全操作快照 |
| `multica issue route --title "..." [--repo <url>] [--project <id>] --output json` | 只读建议项目、重复项、负责人和可用智能体 |
...
| `multica context --output json` | 查看不含密钥的实时工作区、项目、仓库和路由智能体上下文 |
