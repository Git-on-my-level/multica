...
<Callout type="info">
**macOS updater cleanup** — failed Squirrel.Mac installs can leave duplicate `Multica.app.backup-*` folders in `/Applications` and caches under `~/Library/Caches/@multicadesktop-updater`, `~/Library/Caches/ai.multica.desktop`, and `~/Library/Caches/ai.multica.desktop.ShipIt`. Those copies show up in Spotlight. Quit Multica, then run `bash scripts/cleanup-macos-desktop-updater.sh` from the repo (or delete those paths manually) before reinstalling from a fresh DMG.
</Callout>
