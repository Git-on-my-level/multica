...
| `multica issue url <id>` | 設定済みアプリ URL を使ったワークスペース付きイシュー URL を表示 |
| `multica issue inspect <id> --output json` | 所有者、実行、PR 引き継ぎ、安全な操作のスナップショットを表示 |
| `multica issue route --title "..." [--repo <url>] [--project <id>] --output json` | 変更せずにプロジェクト、重複、所有者、利用可能なエージェントを提案 |
...
| `multica context --output json` | 秘密情報を含まないワークスペース、プロジェクト、リポジトリ、ルーティングエージェントのライブ情報を表示 |
