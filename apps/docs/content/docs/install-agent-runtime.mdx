...
For ZCode, install the ZCode desktop app, sign in once in the app, and install the community ACP bridge [zcode-acp-server](https://github.com/william0wang/zcode-acp) (`npm i -g zcode-acp-server`, **version >= 0.2.0** — the first release with the runtime-prefs handshake fix for ZCode >= 0.16). The bridge drives the runtime the desktop app installed and reuses its credentials, so no separate sign-in is needed.
...
<Callout type="info">
Antigravity requires 1.1.10 or later, Claude Code requires 2.0.0 or later, Codex 0.100.0 or later, Copilot 1.0.0 or later, Grok 0.2.89 or later, Qwen Code 0.20.0 or later, MiniMax Code 0.1.2 or later, and Devin CLI 3000.3.22 or later. Below the minimum version, the daemon does not register the corresponding runtime.
</Callout>
