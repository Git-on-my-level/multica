...
[OMP (oh-my-pi)](https://pi.dev/) is a Pi-derived coding agent. It is **not** wire-compatible with the upstream `pi` CLI's JSON stream mode, so Multica drives it over the standard ACP transport via `omp acp --yolo`, sharing the transport with Hermes, Kimi, Kiro CLI, Qoder, and Trae. Session resumption works through ACP `session/load`, MCP config is passed through ACP `mcpServers`, model selection is discovered dynamically (OMP advertises models as a `configOptions` entry, switched per task via `session/set_config_option` — not `session/set_model`, which OMP rejects), and skills are copied into `.omp/skills/`. omp reads `AGENTS.md` natively, so the runtime brief is not inlined into the prompt.

| | |
|---|---|
| Daemon looks for | `omp` |
| Install | See the project at [pi.dev](https://pi.dev/). |
| Authentication | Configure a model provider (Anthropic, OpenAI, etc.) via omp's own config (`~/.omp/agent`), or set the relevant `*_API_KEY` environment variable. |
| Notes | Set a daemon-wide default model with `MULTICA_OMP_MODEL`. |
