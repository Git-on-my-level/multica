...
- **Manual linking requires a mirrored PR** — you can link a PR to an issue by hand from the issue's PR panel or with `multica issue pull-requests link <issue> --url <github-pr-url> [--close-intent]` (and unlink the same way). The PR must already be mirrored in the workspace — i.e. a connected GitHub App installation has reported it via webhook. Manually fetching an unmirrored PR through the installation is not supported yet; a PR that hasn't appeared in Multica returns an actionable error. A manual link with **close intent** to an already-merged PR moves the issue to `Done` under the same native gate as a `Closes` keyword (no open sibling PRs, issue not already `done`/`cancelled`). Unlinking never reopens a `done`/`cancelled` issue.
