...
| `multica issue url <id>` | 설정된 앱 URL을 사용한 워크스페이스 포함 이슈 URL 출력 |
| `multica issue inspect <id> --output json` | 소유자, 실행, PR 인계 및 안전 작업 스냅샷 조회 |
| `multica issue route --title "..." [--repo <url>] [--project <id>] --output json` | 변경 없이 프로젝트, 중복, 소유자 및 사용 가능한 에이전트 제안 |
...
| `multica context --output json` | 비밀 정보를 제외한 워크스페이스, 프로젝트, 저장소 및 라우팅 에이전트의 실시간 컨텍스트 표시 |
