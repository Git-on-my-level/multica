...
// It needs a signed-in ZCode install on the host and never runs in CI.
//
// Usage:
//
...
func main() {
	...
	if bridge == "" {
		// zcode-acp-server from `npm i -g zcode-acp-server` (>= 0.2.0),
		// resolved through PATH like the daemon probe does.
		bridge = "zcode-acp-server"
	}
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelInfo}))

	env := map[string]string{}
	// ZCODE_BIN optionally points the bridge at a specific ZCode runtime
	// (the desktop app's bundled one by default on macOS).
	if bin := os.Getenv("ZCODE_E2E_ZCODE_BIN"); bin != "" {
		env["ZCODE_BIN"] = bin
	}
