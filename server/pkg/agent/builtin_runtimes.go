...
// Fork overlay: OMP is ACP-native (`omp acp`), not the upstream
// oh-my-pi JSON-stream / pi-family assumption. ProtocolFamily is "omp" so
// ResolveBackend/NewRuntime dispatch to ompBackend.
var BuiltinRuntimes = []BuiltinRuntime{
	{
		...
		LaunchHeader:      "omp acp",
		DefaultExecutable: "omp",
		ProviderLabel:     "omp",
		ModelDiscovery:    discoverOmpModels,
	},
	{
		// Fork overlay: host-local Devin CLI over ACP. Not cloud Devin.
		// ProtocolFamily is "devin" so ResolveBackend/New dispatch to
		// devinBackend. No root --permission-mode; auto-approve via ACP.
		ID:                "devin",
