...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- You delivered what the issue itself asks for and it awaits acceptance → `in_review`. Delivering an issue assigned to you — including a sub-issue in a chain or stage — always lands here; this durably asks the parent owner to review the delivery. Review attention does not close a stage or authorize advancing it. `done` stays human.\n")
