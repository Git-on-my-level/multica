...
func TestWorkspaceEventCaptureInstallsAfterPrerequisiteIndexes(t *testing.T) {
	...
	tableMigration := read("474_workspace_event_outbox.up.sql")
	...
	for _, name := range []string{
		"475_workspace_event_cursor_unique.up.sql",
		"476_workspace_event_source_unique.up.sql",
		"477_workspace_event_sequence_unique.up.sql",
		"480_workspace_event_id_unique.up.sql",
	} {
	...
	captureMigration := read("481_workspace_event_capture.up.sql")
	for _, required := range []string{"CREATE OR REPLACE FUNCTION append_workspace_event", "CREATE TRIGGER capture_issue_workspace_event_trigger", "CREATE TRIGGER capture_artifact_workspace_event_trigger"} {
		if !strings.Contains(captureMigration, required) {
			t.Errorf("migration 481 missing %q", required)
		}
	}
	retentionIndex := strings.TrimSpace(read("482_workspace_event_retention_index.up.sql"))
