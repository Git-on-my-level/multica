...
func TestWorkspaceEventCaptureInstallsAfterPrerequisiteIndexes(t *testing.T) {
	...
	tableMigration := read("408_workspace_event_outbox.up.sql")
	if strings.Contains(tableMigration, "CREATE TRIGGER") || strings.Contains(tableMigration, "append_workspace_event") {
		t.Fatal("migration 408 installs capture before prerequisite unique indexes")
	}
	for _, name := range []string{
		"409_workspace_event_cursor_unique.up.sql",
		"410_workspace_event_source_unique.up.sql",
		"411_workspace_event_sequence_unique.up.sql",
		"414_workspace_event_id_unique.up.sql",
	} {
	...
	captureMigration := read("415_workspace_event_capture.up.sql")
	for _, required := range []string{"CREATE FUNCTION append_workspace_event", "CREATE TRIGGER capture_issue_workspace_event_trigger", "CREATE TRIGGER capture_artifact_workspace_event_trigger"} {
		if !strings.Contains(captureMigration, required) {
			t.Errorf("migration 415 missing %q", required)
		}
	}
	retentionIndex := strings.TrimSpace(read("416_workspace_event_retention_index.up.sql"))
