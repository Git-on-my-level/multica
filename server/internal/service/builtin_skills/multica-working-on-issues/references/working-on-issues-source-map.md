...
| Terminal child statuses for barriers are **only** `done` and `cancelled` (`in_review`/`blocked` do not wake parent) | `server/internal/handler/issue_child_done.go` `isTerminalChildStatus` | product contract |
