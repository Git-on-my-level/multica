...
| Stage-terminal child statuses are `done`, `cancelled`, and `in_review`; `blocked` is immediate attention wake (not stage-terminal) | `server/internal/handler/issue_child_done.go` `isTerminalChildStatus` / `notifyParentOfChildBlocked` | fork product contract |
